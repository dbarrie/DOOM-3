...
public:
	...
	FglBuffer			GetAPIObject() const { return m_apiObject; }
...
protected:
	...
	FglBuffer			m_apiObject;
	FglDeviceMemory		m_memory;
...
public:
	...
	void* MapBuffer(bufferMapType_t mapType);
	uint16_t* MapIndexBuffer(bufferMapType_t mapType) { return static_cast<uint16_t*>(MapBuffer(mapType)); }
	void				UnmapBuffer();
...
private:
	idUniformBuffer(const idUniformBuffer&);
	void operator=(const idUniformBuffer&);
};
