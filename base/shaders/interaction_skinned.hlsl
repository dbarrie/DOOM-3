...
[[vk::binding(0)]] ConstantBuffer<Constants> consts;

struct BoneMats
{
	float4 matrices[408];
};
[[vk::binding(1)]] ConstantBuffer<BoneMats> bones;

[[vk::combinedImageSampler]][[vk::binding(1)]] Texture2D<float4> bumpTexture;
[[vk::combinedImageSampler]][[vk::binding(2)]] Texture2D<float4> lightFalloffTexture;
[[vk::combinedImageSampler]][[vk::binding(3)]] Texture2D<float4> lightProjTexture;
[[vk::combinedImageSampler]][[vk::binding(4)]] Texture2D<float4> diffuseTexture;
[[vk::combinedImageSampler]][[vk::binding(5)]] Texture2D<float4> specularTexture;

[[vk::combinedImageSampler]][[vk::binding(1)]] SamplerState bumpSampler;
[[vk::combinedImageSampler]][[vk::binding(2)]] SamplerState lightFalloffSampler;
[[vk::combinedImageSampler]][[vk::binding(3)]] SamplerState lightProjSampler;
[[vk::combinedImageSampler]][[vk::binding(4)]] SamplerState diffuseSampler;
[[vk::combinedImageSampler]][[vk::binding(5)]] SamplerState specularSampler;
...
VsPsInterp MainVs(VsInput input)
{
	...
	float3 toLight = consts.rpLocalLightOrigin.xyz - input.position;
	...
	output.texcoord1.x = dot(float4(input.texcoord, 0.0f, 1.0f), consts.rpBumpMatrixS);
	output.texcoord1.y = dot(float4(input.texcoord, 0.0f, 1.0f), consts.rpBumpMatrixT);
	
	output.texcoord2 = kDefaultTexcoord;
	output.texcoord2.x = dot(position, consts.rpLightFalloffS);
	
	output.texcoord3.x = dot(position, consts.rpLightProjectionS);
	output.texcoord3.y = dot(position, consts.rpLightProjectionT);
	output.texcoord3.z = 0.0f;
	output.texcoord3.w = dot(position, consts.rpLightProjectionQ);
	
	output.texcoord4 = kDefaultTexcoord;
	output.texcoord4.x = dot(float4(input.texcoord, 0.0f, 1.0f), consts.rpDiffuseMatrixS);
	output.texcoord4.y = dot(float4(input.texcoord, 0.0f, 1.0f), consts.rpDiffuseMatrixS);
	
	output.texcoord5 = kDefaultTexcoord;
	output.texcoord5.x = dot(float4(input.texcoord, 0.0f, 1.0f), consts.rpSpecularMatrixS);
	output.texcoord5.y = dot(float4(input.texcoord, 0.0f, 1.0f), consts.rpSpecularMatrixS);
	
	toLight = normalize(toLight);
	float3 toView = normalize(consts.rpLocalViewOrigin - input.position);
...
float4 MainPs(VsPsInterp input) : SV_Target0
{
	...
	float3 diffuseColor = diffuseMap.xyz * consts.rpDiffuseModifier.xyz;
	float3 specularColor = specMap.xyz * rpSpecularModifier.xyz * specPow;
