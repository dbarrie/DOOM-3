...
VsPsInterp MainVs(VsInput input)
{
	...
	output.texcoord0 = float4(input.texcoord.xy, 0, 0) + vsConsts.rpUser0;
	...
	x /= w;
	x = min(x, 0.02f);
	output.texcoord1 = x * vsConsts.rpUser1;
...
{
	float4 bumpMap = Texture1.Sample(Sampler1, input.texcoord0.xy) * 2.0f - 1.0f;
	float2 localNormal = bumpMap.wy;
	
	float2 screenTexCoord = input.position.xy * psConsts.rpWindowCoord.xy;
	screenTexCoord += localNormal * input.texcoord1.xy;
